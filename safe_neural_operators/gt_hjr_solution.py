# Ground truth hjr solution for neural operator 
import os
...
class GroundTruthHJSolution:
    ...
    def __init__(self, hj_dynamics, solve=True, num_time_steps=5, grid_resolution=None):
        ...
        if self.loss_type == 'brt_hjivi':
            ...
            if self.set_mode == 'avoid': 
                ...
                # Max with Min avoid value: Prevents NaNs and values from blowing up
                min_obstacle_value = jnp.min(self.avoid_values)
                brt = lambda obstacle: (lambda t, x: jnp.maximum(jnp.minimum(x, obstacle), min_obstacle_value))

                self.postprocessor = brt(self.avoid_values)
                ...
                self.postprocessor = brt(self.reach_values)
            ...
            self.postprocessor = brat(self.avoid_values, self.reach_values)
    ...
    def solve_hjr(self, ): 
        solver_settings = hj.SolverSettings.with_accuracy("very_high", 
                                                        value_postprocessor=self.postprocessor)
